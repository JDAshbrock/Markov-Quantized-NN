...
for l=1:looplength%Training Loop
	...
	%Feed Forward
	Layer1Outputs = horzcat(pt_relu(Layer1Weights*Im),1)'; %The concatenated one takes care of the bias
	...
	for i=1:Layer1Size %#ok<ALIGN>
		for j=1:InputSize %#ok<ALIGN>
            ...
            if(temp>=0)
                Layer1Gradient(i,j)= NeuralOutputGradients(i)*Im(j);
            end
        end
    end
    %Update
    ...
    if(rem(l,1000)==0)
        ...
        Layer1Outputs = horzcat(pt_relu(Layer1Weights*Im),1)';
        FinalOutputs = FinalWeights*Layer1Outputs;
